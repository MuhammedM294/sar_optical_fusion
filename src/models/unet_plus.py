...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.5):
        ...
        if dropout > 0:
            self.double_conv.append(nn.Dropout(p=dropout))
...
class UNetPlusPlus(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, deep_supervision=False, **kwargs):
        super().__init__()
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            return torch.mean(torch.stack([output1, output2, output3, output4], 0), 0)
